...
(() => {
  ...
  const storage = {
    ...
    set(key,value){ try { window.localStorage.setItem(key,value); } catch { /* storage may be unavailable */ } },
    remove(key){ try { window.localStorage.removeItem(key); } catch { /* storage may be unavailable */ } }
  };
  ...
  function writeJSON(key, value) {
    try { storage.set(key, JSON.stringify(value)); } catch { /* private mode may deny storage */ }
  }
  ...
  els.speechRate.value = storage.get(KEYS.rate) || "0.92";
  els.speechRate.addEventListener("change", () => {
    try { storage.set(KEYS.rate, els.speechRate.value); } catch { /* ignore */ }
  });
